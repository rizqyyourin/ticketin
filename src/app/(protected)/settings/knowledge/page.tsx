...
import { BookOpen, Search, ChevronUp, ChevronDown, ChevronsUpDown, MoreVertical, Archive, Pencil, Trash2, Hash, FileText, Tag, Ticket, Activity } from "lucide-react";
import {
  MOCK_ARTICLES,
  STATUS_STYLES,
  type Article,
  ...
  return (
    <PageShell>
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => router.push("/settings/knowledge/new")} className="cursor-pointer flex items-center gap-2 px-4 py-2 rounded-xl bg-primary text-white text-sm font-semibold hover:bg-primary/90 transition-colors">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-zinc-50 dark:bg-zinc-800/50 border-b border-zinc-100 dark:border-zinc-800">
              <tr>
                <th className={th} onClick={() => handleSort("articleId")}><div className="flex items-center gap-1.5"><Hash className="size-3.5 text-zinc-400" />Article ID <SortIcon field="articleId" sortField={sortField} sortDir={sortDir} /></div></th>
                <th className={th} onClick={() => handleSort("title")}><div className="flex items-center gap-1.5"><FileText className="size-3.5 text-zinc-400" />Title <SortIcon field="title" sortField={sortField} sortDir={sortDir} /></div></th>
                <th className={th} onClick={() => handleSort("type")}><div className="flex items-center gap-1.5"><Tag className="size-3.5 text-zinc-400" />Type <SortIcon field="type" sortField={sortField} sortDir={sortDir} /></div></th>
                <th className={th} onClick={() => handleSort("ticketType")}><div className="flex items-center gap-1.5"><Ticket className="size-3.5 text-zinc-400" />Ticket Type <SortIcon field="ticketType" sortField={sortField} sortDir={sortDir} /></div></th>
