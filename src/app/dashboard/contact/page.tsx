  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => router.push("/dashboard/contact/new")} className="cursor-pointer flex items-center gap-2 px-4 py-2 rounded-xl bg-primary text-white text-sm font-semibold hover:bg-primary/90 transition-colors">
          ...
          <table className="w-full">
            <thead className="bg-zinc-50 dark:bg-zinc-800/50 border-b border-zinc-100 dark:border-zinc-800">
              <tr>
                <th className={thClass} onClick={() => handleSort("title")}>
                  <div className="flex items-center gap-1.5">Title <SortIcon field="title" sortField={sortField} sortDir={sortDir} /></div>
                </th>
                <th className={thClass} onClick={() => handleSort("customerName")}>
                  <div className="flex items-center gap-1.5">Customer Name <SortIcon field="customerName" sortField={sortField} sortDir={sortDir} /></div>
                </th>
                <th className={thClass} onClick={() => handleSort("phone")}>
                  <div className="flex items-center gap-1.5">Phone Number <SortIcon field="phone" sortField={sortField} sortDir={sortDir} /></div>
                </th>
                <th className={thClass} onClick={() => handleSort("email")}>
                  <div className="flex items-center gap-1.5">Email Address <SortIcon field="email" sortField={sortField} sortDir={sortDir} /></div>
                </th>
                <th className={thClass} onClick={() => handleSort("organization")}>
                  <div className="flex items-center gap-1.5">Organization <SortIcon field="organization" sortField={sortField} sortDir={sortDir} /></div>
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
              {visible.length === 0 ? (
                ...
                visible.map((contact) => (
                  <tr key={contact.id} onClick={() => router.push(`/dashboard/contact/${contact.id}`)} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors cursor-pointer">
